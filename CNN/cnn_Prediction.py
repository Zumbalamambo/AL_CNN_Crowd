...
def cnn_predict(filename, test_datasets = [], nkerns=[20, 50, 50], batch_size=500, seed=8000):
    ...
    construct_W_image(classifier_2.__getstate__())
    ...
    for it in range(len(test_datasets)):
        ...
        ppm = theano.function([index], classifier_2.layer5.pred_probs(),
                              givens={
    ...
    # kernals = classifier_2.layer2.get_conv_value()
    ...
    flatted_representaions = classifier_2.get_flatted_params()
...
def visulaize_cov_images(s):
    ...
    for i in xrange(len(s[0])):
        ...
        plt.imshow(s[0][i].reshape((6, 6)), cmap=cm.Greys_r)
        plt.grid(False)
